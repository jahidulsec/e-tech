...
>(({ className, variant = "outline", size = "icon", ...props }, ref) => {
  ...
  return (
    <div className="absolute left-[50%] bottom-10 -translate-x-[50%] flex gap-3 items-center">
      {
        Array.from({length: count}, (_, i) => i+1).map((item) => (
          <button type="button" className={`${current === item ? `w-8 bg-secondary` :`w-3 bg-accent`} transition-all duration-500 h-3 rounded-full`} key={item}>
